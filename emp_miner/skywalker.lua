...
function emp11.checkWeight()
	...
	if user_id then
		...
		return vRP.computeInvWeight(user_id)+vRP.getInventoryItemAmount(itemName)*ammount[source] <= vRP.getBackpack(user_id) and vRP.tryGetInventoryItem(user_id,configminer.stone,configminer.ammountstone)
...
function emp11.collectOres()
	...
	if user_id then
		...
		TriggerClientEvent('Notify', source, 'sucesso', 'Concluido porém sua <b>Ponta de Britadeira</b> quebrou.', 5000)
		ammount[source] = nil
	end
end
