...
RegisterNUICallback("iniciartrampo",function(data,cb)
	for _,v in pairs(coordenadas) do
		...
		TriggerEvent("Notify","sucesso","Você entrou em serviço.",3000)
						...
						if IsVehicleModel(lastVehicle, GetHashKey(carro_emprego)) and not IsPedInAnyVehicle(PlayerPedId())  then
								...
								if distance <= 2 then
									...
									if destino == quantidade_de_blips then
										...
										TriggerEvent("progress",5000,"Plantando")
										Wait(5000)
	...
	while true do
		...
		if emservico then
			...
			if IsControlJustPressed(0,168) then
				...
				SendNUIMessage({ action = "hideMenu" })
				StopScreenEffect("MenuMGSelectionIn")
				TriggerEvent("Notify","aviso","Você saiu de serviço.",3000)
			end
		end
		Citizen.Wait(slep)
	end
