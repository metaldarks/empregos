	...
	window.addEventListener("message",function(event){
		...
		switch(event.data.action){
			case "showMenu":
				...
				$("#exp").html(event.data.exp + "/2000 exp");

				$("#dinheirorecebido").html("$ 0");
				$("#check").html("0/27");

				let value = event.data.exp * 100 / 2000
			...
			case "atualizar":
				...
				$("#check").html(event.data.checkpoint + "/27");
				$(".text-level3").html(event.data.exp + "/2000");

		

				let value2 = event.data.exp * 100 / 2000
...
$(document).on("click",".iniciar", function() {
		...
		$.post("http://empMotorista/iniciartrampo",JSON.stringify({
			...
			if(data.retorno == 'iniciou') {
				...
				$(".text-level3").html(data.exp + "/2000");

				let valueexp = data.exp * 100 / 2000
